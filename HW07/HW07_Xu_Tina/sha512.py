...
def sha512(plaintext, ciphertext):
    #initialization, those are from the lecture note 15
    ...
    input = open(plaintext, "r")
    message = input.readlines()[0]
    
    bv = BitVector(textstring=message)
    #The following steps are basically from given python file SHA256 and modified some permutation parts to 64 bit
    #step1
    length = bv.length()
    ...
    file.write(message_hash.get_bitvector_in_hex())
    return
